...
\begin{itemize}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item if $ \bot \in A_{i} $ then contradicts hA
		\end{itemize}
	\end{itemize}
	\item for every $i \in I \setminus I'$ show $y_i = 0$
	\begin{itemize}
		\item from our $I'$ we have either $ b_i = \top $ or $ \bot \in A_{i} $
		...
		\begin{itemize}
			...
			(we need $\bot \notin b$ here)
			\item if $ \bot \in A_{i} $ hence $ \top \in (-A^T)_{\star, i} $ then $ ((-A^T)\; y)_j = \top > 0$ \\
			(we use hAT here --- TODO necessary?)
		\end{itemize}
	\end{itemize}
	...
	suffices: $\; \sum\limits_{i \in I \setminus I'} y_i \cdot b_i = 0 $
	\begin{itemize}
		\item $y_i = 0$
		\item $\bot \notin b$
	\end{itemize}
